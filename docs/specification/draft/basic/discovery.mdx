...
for metadata like name and description, catalogs MAY omit these fields and let clients
retrieve them from the server card. Including them can improve listing UX by avoiding
extra round-trips.

| Member        | Type     | Required | Description                                                        |
| :------------ | :------- | :------- | :----------------------------------------------------------------- |
| `displayName` | string   | No       | A human-readable name for the server                               |
